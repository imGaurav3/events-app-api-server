    ...
    stages {
        stage('Get Source') {
            ...
            steps {
                ...
                git branch: 'main',
                    url: 'https://github.com/imGaurav3/events-app-api-server.git'
        ...
        stage('Run Tests') {
         ...
         stage('Build and deploy the container') {
            ...
            steps {
                echo "submit gcr.io/dtc-nov21-user319/api-server-image:v2.${env.BUILD_ID}"
                sh "gcloud builds submit -t gcr.io/dtc-nov21-user319/api-server-image:v2.${env.BUILD_ID} ."
                echo 'Get cluster credentials'
                sh 'gcloud container clusters get-credentials cluster-1 --zone us-central1-c --project dtc-nov21-user319'
                echo "Update the image to use gcr.io/dtc-nov21-user319/api-server-image:v2.${env.BUILD_ID}"
                sh "kubectl set image deployment/demo-api demo-api=gcr.io/dtc-nov21-user319/api-server-image:v2.${env.BUILD_ID} --record"
            }
        }            
    }
}
